...
void run_gemm_tests(int seed, int m, int k, int n, int batch_size, int rep) {
  ...
  run_test(rep, 2.0 * m * n * k * batch_size, [&] {
    for (int bs = 0; bs < batch_size; bs++) {
      ...
      gemm(ta_str, tb_str, m, n, k, E(1), dataA.data() + (bs * m * k), lda,
           dataB.data() + (bs * n * k), ldb, E(1), refC.data() + (bs * m * n),
           m);
    }
...
#define TARGNOLOCAL(_cls, _tir, _tic, _twr, _twc)                              \
  ...
  if ((ex.get_device_type() !=
       blas::sycl_device_property::device_type::SYCL_RCAR_CVENGINE) &&
      (ex.get_device_type() !=
       blas::sycl_device_property::device_type::SYCL_RCAR_HOST_CPU)) {
#if !defined(RCAR)
    ...
    test<ReferenceGemmFactory<data_t, data_t, 256, ta, tb, E>>(rep, ARG);
    test<ReferenceGemmFactory<data_t, data_t, 64, ta, tb, E>>(rep, ARG);
#endif
  } else {
#if defined(RCAR)
