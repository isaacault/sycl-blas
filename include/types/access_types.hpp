...
class Access {
 private:
  ...
  static sycl_blas_inline Layout layout_from_transposition(Transposition &t) {
    if (t.isNormal()) {
  ...
  static sycl_blas_inline Layout combined_layouts(Access &a, Access &b) {
 ...
 public:
  ...
  const sycl_blas_inline bool isRowMajor() const {
    return (layout == Layout::RowMajor);
  }
  const sycl_blas_inline bool isColMajor() const {
